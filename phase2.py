    ...
    if check_password():
        ...
        if not all_bills and not all_inventory_logs:
            ...
            if not daily_sales.empty or not daily_purchases.empty:
                ...
                daily_summary = daily_summary.rename(columns={'index': 'business_date'})
                ...
                melted_summary = pd.melt(daily_summary, id_vars=['business_date'], value_vars=['Sales', 'Purchases'],
                                         var_name='Transaction Type', value_name='Amount (₹)')
                fig = px.bar(melted_summary, x='business_date', y='Amount (₹)', color='Transaction Type', barmode='group',
                             labels={'business_date': 'Date', 'Amount (₹)': 'Amount (₹)', 'Transaction Type': 'Transaction Type'})
...
# --- Daily Report Page ---
elif st.session_state.page == "Daily Report":
    ...
    yesterdays_bills = list(bills_collection.find({
        "timestamp": {
            "$gte": start_of_report_period,
            "$lt": end_of_report_period
        }
    }, {'_id': 0}))
